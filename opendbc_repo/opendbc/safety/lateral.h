...
bool steer_torque_cmd_checks(int desired_torque, int steer_req, const TorqueSteeringLimits limits) {
  ...
  if (!(controls_allowed || lateral_controls_allowed) && torque_requested) {
    violation = true;
  }
...
bool steer_curvature_cmd_checks(int desired_curvature, int steer_power, bool steer_control_enabled, const CurvatureSteeringLimits limits) {
  ...
  if (limits.max_steer_power != 0) {
    ...
    violation |= !(controls_allowed || lateral_controls_allowed) && (steer_power != 0) && (steer_power >= curvature_state.steer_power_last);
