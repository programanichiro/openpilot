...
class HudRenderer(Widget):
  ...
  def _ip_button_init(self):
    ...
    self.red_signal_chk_timing = 3 if not gui_app.big_ui() else 9 #c4は60fpsなので、三倍。
  ...
  def _ip_draw(self, rect: rl.Rectangle):
    ...
    if gui_app.big_ui():
      ...
      debug_disp_xpos = self._drawTextRight(self._font_semi_bold , font_size_debug_info , debug_disp_xpos -10 , rect_h0+4 , cv_str , 140 , False , 0, 0, 0 , 0xdf, 0xdf, 0x00, 200 , 5 , 0.3 , bk_add_w=11-4 , bk_xofs=0 , bk_add_h=-5)
      debug_disp_xpos = self._drawTextRight(self._font_JP , font_size_debug_info , debug_disp_xpos -8 , rect_h0+4 , "↓" , 200 , False , 0xdf, 0xdf, 0x00 , 0, 0, 0, 140 , 5 , 0.3 , bk_add_w=11-4 , bk_xofs=0 , bk_add_h=-5)

      ahr_str = str(int(self.ahr)) + "%"
      debug_disp_xpos = self._drawTextRight(self._font_semi_bold , font_size_debug_info , debug_disp_xpos-8 , rect_h0+4 , ahr_str , 140 , False , 0, 0, 0 , 0xdf, 0xdf, 0x00, 200 , 5 , 0.3 , 4 , 0 , -5)
      debug_disp_xpos = self._drawTextRight(self._font_semi_bold , font_size_debug_info , debug_disp_xpos-4 , rect_h0+4 , "AP" , 200 , False , 0xdf, 0xdf, 0x00 , 0, 0, 0, 140 , 5 , 0.3 , bk_add_w=4-2 , bk_xofs=0-1 , bk_add_h=-5)
      ...
      debug_disp_xpos = self._drawTextRight(self._font_semi_bold , font_size_debug_info , debug_disp_xpos-6-4 , rect_h0+4 , "Trip" , 200 , False , 0xdf, 0xdf, 0x00 , 0, 0, 0, 140 , 5 , 0.3 , bk_add_w=10+1 , bk_xofs=-3 , bk_add_h=-5)

      if abs(self.vc_speed) < 0.1/3.6:
        blue_signal_chk_str = str(self.blue_signal_chk)
        debug_disp_xpos = self._drawTextRight(self._font_semi_bold , font_size_debug_info , debug_disp_xpos -10 , rect_h0+4 , blue_signal_chk_str , 140 , False , 0, 0, 0 , 0xdf, 0xdf, 0x00, 200 , 5 , 0.3 , bk_add_w=13-6 , bk_xofs=1-2 ,bk_add_h=-5)
        debug_disp_xpos = self._drawTextRight(self._font_JP , font_size_debug_info , debug_disp_xpos-8 , rect_h0+4 , "●" , 200 , False , 0xdf, 0xdf, 0x00 , 0, 0, 0, 140 , 5 , 0.3 , bk_add_w=11, bk_xofs=0-4 , bk_add_h=-5)
