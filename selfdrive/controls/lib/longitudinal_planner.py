...
class Planner:
  ...
  def update(self, sm):
    ...
    md = sm['modelV2']
    global signal_scan_ct,path_x_old_signal,path_x_old_signal_check
    if v_ego <= 0.01/3.6 and (OP_ENABLE_v_cruise_kph > 0 or one_pedal == False or (OP_ENABLE_v_cruise_kph == 0 and sm['radarState'].leadOne.status == False)) and sm['controlsState'].enabled and sm['carState'].gasPressed == False: #and (sm['radarState'].leadOne.status == False or (sm['radarState'].leadOne.dRel > 40 and sm['radarState'].leadOne.modelProb > 0.5)):
