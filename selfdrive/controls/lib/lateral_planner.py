...
class LateralPlanner:
  ...
  def update(self, sm):
    ...
    if self.use_lanelines:
      ...
      d_path_xyz = self.LP.get_d_path(STEER_CTRL_Y , (-max_yp / 2.5) , ypf , v_ego, self.t_idxs, self.path_xyz)
      self.lat_mpc.set_weights(MPC_COST_LAT.PATH, MPC_COST_LAT.HEADING, MPC_COST_LAT.STEER_RATE)
    else:
      d_path_xyz = self.path_xyz
      dcm = self.LP.calc_dcm(STEER_CTRL_Y, (-max_yp / 2.5) , ypf , v_ego,2.5,-1,-1) #2.5はレーンを消すダミー,-1,-1はカメラオフセット反映に必要
