    ...
    try:
      with open('/tmp/limitspeed_info.txt','r') as fp:
        ...
        if limitspeed_info_str:
          ...
          if self.velocity >= limitspeed_min and (self.get_limit_avg * 0.9 < self.velocity or self.get_limitspeed_old == 0) and ((int(self.get_limit_avg/5) * 5) != int(self.velocity/5) * 5 or self.get_limitspeed_old == 0 or (((self.min_distance_old**0.5) * 100 / 0.0009 > limit_m or self.min_distance_old == 0) and self.velo_ave_ct_old < 10)):
