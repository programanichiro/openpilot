...
class ConfidenceBall(Widget):
  def __init__(self, demo: bool = False):
    ...
    self._lp1 = gui_app.texture("icons_mici/onroad/acc_dist1_w2.png",width=SIDE_PANEL_WIDTH-8,height=256) #幅をSIDE_PANEL_WIDTH程度に
    self._lp2 = gui_app.texture("icons_mici/onroad/acc_dist2_w2.png",width=SIDE_PANEL_WIDTH-8,height=256)
    self._lp3 = gui_app.texture("icons_mici/onroad/acc_dist3_w2.png",width=SIDE_PANEL_WIDTH-8,height=256)
  ...
  def _render(self, _):
    ...
    if self._LongitudinalPersonality == 0:
      rl.draw_texture(self._lp1,int(content_rect.x),int(content_rect.y + content_rect.height -self._lp1.height-y_ofs), rl.Color(240,240,240,230))
    elif self._LongitudinalPersonality == 1:
      rl.draw_texture(self._lp2,int(content_rect.x),int(content_rect.y + content_rect.height -self._lp2.height-y_ofs), rl.Color(240,240,240,230))
    else: #if self._LongitudinalPersonality == 2:
      rl.draw_texture(self._lp3,int(content_rect.x),int(content_rect.y + content_rect.height -self._lp3.height-y_ofs), rl.Color(240,240,240,230))
