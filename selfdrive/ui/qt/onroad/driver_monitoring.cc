...
void DriverMonitorRenderer::updateState(const UIState &s) {
  ...
  is_active = dm_state.getIsActiveMode(); //MADS時のドライバー監視ロジック追加箇所
...
void DriverMonitorRenderer::draw(QPainter &painter, const QRect &surface_rect) {
  ...
  QColor arc_color = uiState()->engaged() ? DMON_ENGAGED_COLOR : DMON_DISENGAGED_COLOR; //MADS時のドライバー監視ロジック追加箇所
