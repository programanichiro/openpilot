...
//  [STATUS_ALERT] = QColor(0xC9, 0x22, 0x31, 0xf1),
    ...
    if(is_cruise_set){
      ...
      drawTextCenter(p, rect().center().x() + w/2 + 43, 290 + y_ofs-40 , QString::number(ACC_speed) , velo_for_trans < velo_for_trans_limit ? 100 : 255 , false , 0x24, 0x57, 0xa1 , 240, 240, 240, velo_for_trans < velo_for_trans_limit ? 70 : 240 , 9 , 15 , 18 , 2);
    }
  }
...
//以下オリジナル表示要素
  ...
  if (mapVisible == false){
    ...
    drawText(p, traffic_speed_x+traffic_speed_r-2, traffic_speed_y+traffic_speed_r+f_size/2 -7-1, traffic_speed , QColor(0x24, 0x57, 0xa1 , 255));
  }
