  ...
  if (navDisabled != navDisabledNow) {
    ...
    if (map) {
      if(this->mb_width_rate != 0){
        map->setFixedWidth((topWidget(this)->width() * this->mb_width_rate - UI_BORDER_SIZE * (navDisabledNow ? 2 : 1)));
      } else {
        //map->setFixedWidth(topWidget(this)->width() * 0.5 - UI_BORDER_SIZE * (navDisabledNow ? 2 : 1));
...
void OnroadWindow::paintEvent(QPaintEvent *event) {
  ...
  if (isMapVisible() && navDisabled) {
    int w;
    if(this->mb_width_rate != 0){
      w = (int)((width() - UI_BORDER_SIZE * 2) * this->mb_width_rate) + UI_BORDER_SIZE -1;
    } else {
      w = (width() - UI_BORDER_SIZE * 2) /2 + UI_BORDER_SIZE;
    }
...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  if (has_us_speed_limit) {
    ...
    configFont(p, "Inter", 28*max_disp_k, "SemiBold");
  ...
  if (has_eu_speed_limit) {
    ...
    configFont(p, "Inter", (speedLimitStr.size() >= 3) ? 60*max_disp_k : 70*max_disp_k, "Bold");
    p.setPen(blackColor());
...
//以下オリジナル表示要素
  ...
  if (global_engageable) {
    ...
    drawIcon(p, rect().right() - btn_size / 2 - UI_BORDER_SIZE * 2, btn_size / 2 + int(UI_BORDER_SIZE * 1.5)+y_ofs,
