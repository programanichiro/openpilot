...
void MapWindow::updateState(const UIState &s) {
  ...
  if (interaction_counter == 0) {
    ...
    interaction_counter--;
    if(interaction_counter <= 0){ //<=falesafe
      interaction_counter = 0; //falesafe
      before_pinch_angle = 0;
      last_pinch_angle = 0;
    }
  }
  ...
  if (sm.updated("navInstruction")) {
    ...
    if (sm.valid("navInstruction")) {
      ...
      if(now_navigation == false && night_mode >= 0){
        ...
        m_map->setStyleUrl("mapbox://styles/kawombop0/clxpnfmk000ke01pu7kk4ehrt"); //ナビ中はスタイルを公式に戻す。公式＋渋滞情報
        style_reload = 10;
      }
      now_navigation = true;
    } else {
...
void MapWindow::offroadTransition(bool offroad) {
  if (offroad) {
    ...
    if (last_position){
      ...
      std::string lastGPSPosJSON = util::string_format("{\"latitude\": %.15f, \"longitude\": %.15f, \"altitude\": %.15f}", lat, lon, 0/*dummyの高さ*/);
