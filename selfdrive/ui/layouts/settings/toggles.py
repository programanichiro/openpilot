...
DESCRIPTIONS = {
  ...
  "RaylibMode": tr("Switching to a user interface built with raylib."),
  "AccelMethodSwitch": tr("Switch Accel Method to Official version or recommendation. A reboot is required."),
  "GpsAlwaysSwitch": tr("GPS reception starts even when the car is not moving. This speeds up satellite acquisition and prevents GPS reception from being interrupted during temporary Offroad situations. However, it may affect battery consumption when the car is stationary."),
  "DisableMaxSpeedModify": tr("ACC speeds exceeding 115 km/h will be obtained directly from the vehicle. TSSP 2019 PHV users should enable it."),
  "ForceHybridVehicle": tr("Turn this switch on if a hybrid vehicle is incorrectly recognized as a gas vehicle. Do not turn it on for gas vehicles, as this will cause a crash."),
  "IgnoreRerouteHarness": tr("Fix a CAN error on a vehicle that does not have a DSU bypass harness or smartDSU installed."),
}
